...
def load_big_vision_weights(model: CustomTextCLIP, checkpoint_path: str):
    ...
    def _convert_timm_img(module, prefix):
        ...
        if pos_embed_w.shape != module.pos_embed.shape:
            ...
            num_prefix_tokens = (
                0
                if getattr(module, "no_embed_class", False)
                else getattr(module, "num_prefix_tokens", 1)
            )
        ...
        for i, block in enumerate(module.blocks.children()):
            ...
            block.norm1.weight.copy_(
                _n2p(w[f"{block_prefix}LayerNorm_0/scale"], idx=idx)
            )
            block.norm1.bias.copy_(_n2p(w[f"{block_prefix}LayerNorm_0/bias"], idx=idx))
            block.attn.qkv.weight.copy_(
                torch.cat(
                    [
                        _n2p(w[f"{mha_prefix}{n}/kernel"], t=False, idx=idx)
                        .flatten(1)
                        .T
        ...
        if module.attn_pool is not None:
            ...
            module.attn_pool.q.bias.copy_(
                _n2p(w[f"{mha_prefix}query/bias"], t=False).reshape(-1)
            )
            module.attn_pool.kv.weight.copy_(
                torch.cat(
                    [
            ...
            module.attn_pool.kv.bias.copy_(
                torch.cat(
                    [
    ...
    def _convert_openclip_transformer(module: Transformer, prefix):
        for i, block in enumerate(module.resblocks.children()):
            ...
            block.mlp.c_fc.weight.copy_(
                _n2p(w[f"{block_prefix}MlpBlock_0/Dense_0/kernel"])
            )
            ...
            block.mlp.c_proj.bias.copy_(
                _n2p(w[f"{block_prefix}MlpBlock_0/Dense_1/bias"])
            )

    def _convert_openclip_txt(module: TextTransformer, prefix):
        module.token_embedding.weight.copy_(
    ...
    def _convert_openclip_txt(state_dict, prefix="text_encoder."):
        ...
        for k, v in state_dict.items():
            ...
            if k.startswith("positional_embedding.pos_embed.pos_embed"):
                k = k.replace(
                    "positional_embedding.pos_embed.pos_embed", "positional_embedding"
                )
                v = v.squeeze()
