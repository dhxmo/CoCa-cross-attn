...
parser.add_argument(
    "--profiler", default="torch", type=str, choices=["torch", "fvcore"]
)
parser.add_argument(
    "--batch-size", default=1, type=int, help="Batch size for profiling"
)
...
def profile_fvcore(
    ...
    aca = fvcore.nn.ActivationCountAnalysis(
        model, (example_image_input, example_text_input)
    )
    if detailed:
...
def profile_fvcore_text(
    model, text_input_size=(77,), batch_size=1, detailed=False, force_cpu=False
):
    if force_cpu:
        model = model.to("cpu")
...
def profile_fvcore_image(
    model, image_input_size=(3, 224, 224), batch_size=1, detailed=False, force_cpu=False
):
    if force_cpu:
        model = model.to("cpu")
    device, dtype = next(model.parameters()).device, next(model.parameters()).dtype
...
def profile_torch(
    model, text_input_size, image_input_size, batch_size=1, force_cpu=False
):
    """Profile the full model using torch.utils.flop_counter"""
    if force_cpu:
        model = model.to("cpu")
    device, dtype = next(model.parameters()).device, next(model.parameters()).dtype
    image_input = torch.ones(
        (batch_size,) + image_input_size, device=device, dtype=dtype
    )
    text_input = torch.ones(
        (batch_size,) + text_input_size, device=device, dtype=torch.int64
    )
...
def profile_model(model_name, batch_size=1, profiler="torch", device="cuda"):
    assert profiler in [
        "torch",
        "fvcore",
    ], "Only torch and fvcore profilers are supported"
    if profiler == "fvcore":
